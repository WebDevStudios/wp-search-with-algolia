...
class AdminSettings {
	 ...
	 * @since  1.0.0
	 *
	 * @param Plugin $plugin The Algolia_Plugin instance.
	 */
	...
	public function sanitize_api_key( $value ) {
		...
		try {
			Api::assert_valid_credentials( $settings->get_application_id(), $value );
		...
		if ( ! $valid_credentials ) {
			...
			if ( ! Api::is_valid_search_api_key( $settings->get_application_id(), $settings->get_search_api_key() ) ) {
				add_settings_error(
					$this->option_group,
