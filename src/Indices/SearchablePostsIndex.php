 ...
 * @since 1.0.0
 */
final class SearchablePostsIndex extends AbstractIndex {
	...
	private function get_post_records( WP_Post $post ) {
		...
		$post_content = Utils::prepare_content( $post_content );
		$parts        = Utils::explode_content( $post_content );
	...
	private function get_post_shared_attributes( WP_Post $post ) {
		...
		$shared_attributes['images'] = Utils::get_post_images( $post->ID );
		...
		foreach ( $taxonomy_objects as $taxonomy ) {
			...
			if ( $taxonomy->hierarchical ) {
				$hierarchical_taxonomy_values = Utils::get_taxonomy_tree( $terms, $taxonomy->name );
	...
	private function get_post_object_id( $post_id, $record_index ) {
		 ...
		 * @since 1.3.0
		 *
		 * @see PostsIndex::get_post_object_id()
		 */
